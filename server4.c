...
typedef struct {
    ...
    int requestNumber;
    pthread_rwlock_t *rwlock_pointer;  
} RequestParameters; 
char** theArray;
...
void *ProcessRequest(void* args) {
	...
	pthread_rwlock_t *rwlock_array_pointer = param_struct->rwlock_pointer;
	...
	if (rqst_struct->is_read == 0) { // write request
		...
		pthread_rwlock_wrlock(&rwlock_array_pointer[rqst_struct->pos]);
		setContent(rqst_struct->msg, rqst_struct->pos, theArray); 
		pthread_rwlock_unlock(&rwlock_array_pointer[rqst_struct->pos]);
		
		// retrieve and return updated value to client
		char retrieved_string[COM_BUFF_SIZE]; 
		pthread_rwlock_rdlock(&rwlock_array_pointer[rqst_struct->pos]);
...
int main (int argc, char* argv[]) {	
	...
	pthread_rwlock_t rwlocks[array_size];
	...
	if (bind(serverFileDescriptor,(struct sockaddr*)&sock_var_,sizeof(sock_var_))>=0) {
			...
			for (thread = 0; thread < COM_NUM_REQUEST; thread++) {  
				...
				param_struct->rwlock_pointer = &rwlocks[0];
